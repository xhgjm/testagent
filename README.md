...
Phase 2.3.6 新增 runtime workspace alignment：实验性 runtime tool 注入前会解析并创建 `tenant_id/user_id/agent_id/session_id` 隔离 workspace，并把 workspace 上下文字段写入 runtime audit。默认仍关闭，不接 custom WorkspaceManager，不让工具读取或写入文件。

Phase 2.3.7 新增 runtime tool full regression 和 WorkspaceManager alignment design：固化本地 smoke 脚本，核验 AgentScope `WorkspaceManagerBase` / `LocalWorkspaceManager` 生命周期边界，并明确暂不替换 AgentScope `LocalWorkspaceManager`。
...
`runtime_echo_tool` 在显式开启 runtime tools 且 permission allow 后，会绑定平台 workspace 上下文：
...
路径解析复用 Phase 2 的平台 workspace resolver，隔离策略保持 `tenant_id/user_id/agent_id/session_id`。本阶段没有替换 AgentScope `LocalWorkspaceManager`，也没有实现 custom WorkspaceManager；`runtime_echo_tool` 仍只回显文本，不读写 workspace 文件，不把 `workspace_path` 返回给模型。

完整说明见 [docs/phase2_3_6-runtime-workspace-alignment.md](docs/phase2_3_6-runtime-workspace-alignment.md)。

## Phase 2.3.7 Runtime Regression And WorkspaceManager Design

Phase 2.3.7 是收口和设计阶段，不新增外部 API，不改变 runtime 主链路。新增本地 smoke 脚本：

```bash
python scripts/smoke_phase2_3_7_runtime_tools.py
```

该脚本只使用临时 permission/audit/workspace 文件，验证默认关闭、allow 注入、执行时二次权限、runtime audit、workspace 字段和 tenant 隔离。

AgentScope 2.0.3 `LocalWorkspaceManager` 已核验：本地 workdir 当前为 `basedir/agent_id`，而平台 resolver 是 `basedir/tenant_id/user_id/agent_id/session_id`。本阶段仅记录边界和未来 `PlatformWorkspaceManager` 设计，不替换当前 `LocalWorkspaceManager`。

完整说明见 [docs/phase2_3_7-runtime-tool-full-regression.md](docs/phase2_3_7-runtime-tool-full-regression.md) 和 [docs/phase2_3_7-workspace-manager-alignment-design.md](docs/phase2_3_7-workspace-manager-alignment-design.md)。
...
- Phase 2.3.6：Runtime workspace alignment，复用平台 workspace resolver 并记录 workspace audit fields。
- Phase 2.3.7：Runtime tool full regression 和 AgentScope WorkspaceManager alignment design。
