...
`backend/app/main.py` 在 Phase 3.1 中显式传入禁用状态的 AgentScope RAG 参数：
...
这不是从环境变量动态读取的值。`enable_index_worker` 不绑定 `settings.platform_rag_enable_index_worker`。
...
AgentScope 原生 `/knowledge_bases` router 可能仍存在于 OpenAPI 中，因为它是 AgentScope `create_app` 的内置 router；但在 `knowledge_base_manager=None` 时，原生 RAG Service 没有可用 manager，预期会返回 `503 Service Unavailable`。Phase 3.1 不把它作为企业入口，也不调用它。

Phase 3.1 不注册 `/api/platform/knowledge-bases`。
...
如果输出包含 AgentScope 原生 `/knowledge_bases`，这不代表平台 RAG facade 已启用。验收重点是不得出现 `/api/platform/knowledge-bases`。
