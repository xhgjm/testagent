...
当前阶段：Phase 3.2 KnowledgeBase Facade Skeleton。后端已完成 AgentScope RAG 相关签名核验、平台 RAG facade 设计、RAG 配置骨架、安全状态展示和 KnowledgeBase metadata facade，但尚未启用真实 RAG、Qdrant、embedding、BlobStore、index worker 或 chat RAG。
...
- 下一步进入 Phase 3.3 Document metadata / upload facade skeleton，继续默认关闭真实 RAG runtime。
- Document metadata、Agent-KB binding、RAG permission、RAG audit 仍未实现。
